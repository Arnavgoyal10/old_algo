...
def worker(params, ret):    
    trade_columns = ['entry_time', 'entry_price', 'exit_time', 'exit_price', 'profit', 'agg_profit', 'percent']
...
def final(name):
    ...
    def optimize_function(config):
        ...
        output_dir = '/home/arnav.goyal_ug2023/old_algo/min5_agg/out'
    ...
    output_best_file = os.path.join('min5_agg', f"{name}_runs.csv")
    ...
    output_best_file = os.path.join('min5_agg', f"{name}_best_params.csv")
