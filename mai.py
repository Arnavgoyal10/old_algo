...
def check_trade(df):
    ...
    if current_time < comparison_time:        
        ...
        if (supertrend_signal == True and df_range["in range"].iloc[-1] == False):
            if (df_squeeze["psi"].iloc[-1] < 80 and df_squeeze["psi"].iloc[-2] < 80 and (df_squeeze["psi"].iloc[-1]-2) < df_squeeze["psi"].iloc[-2]):
...
def main():
    ...
    df_comb_file = os.path.join(current_directory, 'performace1.csv')
