...
import mate_1.trial_okxfinal.refracted_advance as refracted
import csv
from hyperopt import hp, fmin, tpe, Trials
from hyperopt.pyll.base import scope
from datetime import datetime
import threading
import other_indicators.okxfinal as okxfinal

lock = threading.Lock()

def calculate_indicators(df, hyperparameters):
    (entryLength, exitLength, conv_config, length_config, 
     lengthMA_config, lengthSignal_config, fast_config, slow_config, 
     signal_config) = hyperparameters

    df = df.copy()
    df = okxfinal.add_trading_signals(df, entryLength=entryLength, exitLength=exitLength)
...
def final(name):
    ...
    'entryLength': hp.uniform('entryLength', 5, 30),
    'exitLength': hp.uniform('exitLength', 5, 30),
    ...
    # Export to CSV
    columns = ["stoploss", "squee", "exitLength", "entryLength", "conv", "length", "lengthMA", "lengthSignal", "fast", "slow", "signal", "net_profit"]
    with open(f"mate_1/trial_okxfinal/{name}_agg.csv", "w", newline='') as csvfile:
