...
import mate_1.trial_okx.refracted_advance as refracted
import csv
from hyperopt import hp, fmin, tpe, Trials
from hyperopt.pyll.base import scope
from datetime import datetime
import threading
import other_indicators.okx as okx

lock = threading.Lock()

def calculate_indicators(df, hyperparameters):
    (entryLength, conv_config, length_config, 
     lengthMA_config, lengthSignal_config, fast_config, slow_config, 
     signal_config) = hyperparameters

    df = df.copy()
    
    df = okx.add_trading_signals(df, entryLength=entryLength)
...
def final(name):
    ...
    with open(f"mate_1/trial_okx/{name}_agg.csv", "w", newline='') as csvfile:
