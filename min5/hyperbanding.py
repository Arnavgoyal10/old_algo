...
def final(name, count):
    ...
    file_path = f'monthly_5min/{folder}/{name}.csv'
    ...
    trials = Trials()
    best = fmin(fn=lambda params: worker(params, ret), space=space, algo=tpe.suggest, max_evals=23500, trials=trials)
    ...
    with open(f"min5/{folder}/{name}_params.csv", "w", newline='') as csvfile:
