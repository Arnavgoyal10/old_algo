...
import mate_3.trial_super.refracted_advance as refracted
import csv
from hyperopt import hp, fmin, tpe, Trials
from hyperopt.pyll.base import scope
from datetime import datetime
import threading
import other_indicators.supertrend as supertrend

lock = threading.Lock()

def calculate_indicators(df, hyperparameters):
    (period, multiplier, conv_config, length_config, 
     lengthMA_config, lengthSignal_config, fast_config, slow_config, 
     signal_config) = hyperparameters

    df = df.copy()
    df = supertrend.SuperTrend(df, period=period, multiplier=multiplier)
...
def final(name):
    ...
    'period': hp.uniform('period', 5, 30),
    'multiplier': hp.uniform('multiplier', 1, 4),
    ...
    # Export to CSV
    columns = ["stoploss", "squee", "period", "multiplier", "conv", "length", "lengthMA", "lengthSignal", "fast", "slow", "signal", "net_profit"]
    with open(f"mate_3/trial_super/{name}_agg.csv", "w", newline='') as csvfile:
